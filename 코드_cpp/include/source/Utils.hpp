...
C<T> FindCrossPoint(C<T> const& pt1, C<T> const& pt2, C<T> const& pt3, C<T> const& pt4)
{
    ...
    try
    {
        xChild = (((pt1.x * pt2.y) - (pt1.y * pt2.x)) * (pt3.x - pt4.x)) -                                              // x축 분자
                 ((pt1.x - pt2.x) * ((pt3.x * pt4.y) - (pt3.y * pt4.x)));

        yChild = (((pt1.x * pt2.y) - (pt1.y * pt2.x)) * (pt3.y - pt4.y)) -                                              // y축 분자
                 ((pt1.y - pt2.y) * ((pt3.x * pt4.y) - (pt3.y * pt4.x)));

        mother = ((pt1.x - pt2.x) * (pt3.y - pt4.y)) - ((pt1.y - pt2.y) * (pt3.x - pt4.x));                             // 분모
    }
